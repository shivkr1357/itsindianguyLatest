...
export async function GET(request: NextRequest) {
  ...
  if (all && !isBlogAdmin(request)) {
    return jsonNoStore({ error: "Unauthorized" }, 401);
  }
  const key = apiKey();
  const url = all ? `${base}/api/categories/all` : `${base}/api/categories`;
  const res = await fetch(url, {
    next: all ? undefined : { revalidate: 60 },
    cache: all ? "no-store" : undefined,
    headers: all && key ? { Authorization: `Bearer ${key}` } : undefined,
  });
